...
{  int err;
  ...
  if(argc==1)
  { 
      printf(" Correct usage:  ./main ");
      for(int i;i<n_inputvals;i++) printf("%s ",varnames[i]); // Modified by Shunichi
      printf("\n");
      //printf("Example: ./main data1.par\n");// Modified by Shunichi
      exit(1);
  }
          
  if(argc<1+n_inputvals)
  {
  ...
  //err=readVar(argv[1]); // Modified by Shunichi
  err=readVar_str(n_inputvals,varnames,&argv[1]); // Modified by Shunichi
  
  //if(err==-1)     {printf("Can not open the file\n"); exit(1);}
  //else if(err>0)  { printf("Wrong file contents at line %d\n",err);exit(1);}
  if(err>0)  { printf("Wrong input contents at line %d\n",err);exit(1);} // Modified by Shunichi
  ...
  */
  ...
  {  double Xf;
     ...
     if(Omega>0) {
       ...
       double sv,Tx;
       Tx=Mcdm/Xf;
       sv=vSigma(Tx,1.E-5 ,1);
       printf("\nSigma*v at T=%.6E [GeV] is %.6E [cm3 s-1]",Tx,sv*29979245800.0*1e-36); };
